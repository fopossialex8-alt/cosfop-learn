...
import { PlanLiveView } from './views/PlanLiveView';
import { MyStudentsView } from './views/MyStudentsView';
import { TeacherMessagesView } from './views/TeacherMessagesView';
import { TeacherReviewsView } from './views/TeacherReviewsView';
import { TeacherStatisticsView } from './views/TeacherStatisticsView';
import { TeacherCertificationsView } from './views/TeacherCertificationsView';
import { TeacherPaymentsView } from './views/TeacherPaymentsView';
import { TeacherPromotionsView } from './views/TeacherPromotionsView';
import { TeacherResourcesView } from './views/TeacherResourcesView';
import { TeacherSupportView } from './views/TeacherSupportView';
...
export const TeacherApp = ({ user: initialUser, onLogout }: TeacherAppProps) => {
  const [activeSubView, setActiveSubView] = useState<TeacherSubView>('dashboard');
  ...
  const renderContent = () => {
    switch (activeSubView) {
      case 'dashboard': return <TeacherDashboardView user={user} onNavigate={setActiveSubView} theme={theme} />;
      case 'my_courses': return <TeacherCoursesView onNavigate={setActiveSubView} theme={theme} />;
      case 'create_course': return <CreateCourseView onNavigate={setActiveSubView} theme={theme} />;
      case 'plan_live': return <PlanLiveView theme={theme} />;
      case 'my_students': return <MyStudentsView theme={theme} />;
      case 'messages': return <TeacherMessagesView theme={theme} />;
      case 'reviews': return <TeacherReviewsView theme={theme} />;
      case 'statistics': return <TeacherStatisticsView theme={theme} />;
      case 'certifications': return <TeacherCertificationsView theme={theme} />;
      case 'payments': return <TeacherPaymentsView theme={theme} />;
      case 'promotions': return <TeacherPromotionsView theme={theme} />;
      case 'resources': return <TeacherResourcesView theme={theme} />;
      case 'support': return <TeacherSupportView theme={theme} />;
      case 'settings': return <TeacherSettingsView theme={theme} setTheme={setTheme} />;
      case 'profile': return <TeacherProfileView user={user} onUpdateAvatar={handleUpdateAvatar} onNavigate={setActiveSubView} theme={theme} />;
      default: return <TeacherDashboardView user={user} onNavigate={setActiveSubView} theme={theme} />;
    }
  };
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className={`h-20 border-b px-8 flex items-center justify-between sticky top-0 z-30 transition-colors duration-500 ${theme === 'dark' ? 'bg-slate-900 border-white/5' : 'bg-white border-ink/5'}`}>
          <div className="flex-1 max-w-xl relative group">
            ...
            <input 
              type="text" 
              placeholder="Rechercher un élève, un cours, une transaction..." 
          ...
          <div className="flex items-center gap-6">
                    ...
                    {MOCK_NOTIFICATIONS.map((notif) => (
                      <div key={notif.id} className={`p-4 transition-colors flex gap-4 border-b last:border-none cursor-pointer ${theme === 'dark' ? 'hover:bg-white/5 border-white/5' : 'hover:bg-ink/5 border-ink/5'}`}>
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${notif.type === 'live' ? 'bg-red-500/10 text-red-500' : 'bg-secondary/10 text-secondary'}`}>
                          {notif.type === 'live' ? <Zap className="w-5 h-5" /> : <Zap className="w-5 h-5" />}
                        </div>
                        <div>
            ...
            >
              ...
              <div className="text-left hidden sm:block">
                ...
                <p className="text-[10px] font-bold text-primary uppercase tracking-wider mt-1">Enseignant Pro</p>
              </div>
